...
class ModelSerializer(serializers.Serializer):
    class Query(serializers.Serializer):
        ...
        model_type = serializers.CharField(
            required=False, error_messages=ErrMessage.char(_("model type"))
        )

        model_name = serializers.CharField(
            required=False, error_messages=ErrMessage.char(_("model name"))
        )

        provider = serializers.CharField(
            required=False, error_messages=ErrMessage.char(_("provider"))
        )

        permission_type = serializers.CharField(
            required=False, error_messages=ErrMessage.char(_("permission type"))
        )

        create_user = serializers.CharField(
            required=False, error_messages=ErrMessage.char(_("create user"))
        )
            ...
            if create_user is not None:
                ...
                # 当前用户能查看其他人的模型，只能查看公开的
                    ...
                    model_query_set = QuerySet(Model).filter(
                        (
                            Q(user_id=self.data.get("create_user"))
    ...
    class Edit(serializers.Serializer):
        ...
        permission_type = serializers.CharField(
            required=False,
            error_messages=ErrMessage.char(_("permission type")),
            validators=[
                validators.RegexValidator(
                    regex=re.compile("^PUBLIC|PRIVATE$"),
                    message=_("permissions only supportPUBLIC|PRIVATE"),
                    code=500,
                )
            ],
        )
    ...
    class Create(serializers.Serializer):
        ...
        provider = serializers.CharField(
            required=True, error_messages=ErrMessage.char(_("provider"))
        )

        model_type = serializers.CharField(
            required=True, error_messages=ErrMessage.char(_("model type"))
        )
        ...
        model_name = serializers.CharField(
            required=True, error_messages=ErrMessage.char(_("model name"))
        )
        ...
        def is_valid(self, *, raise_exception=False):
            ...
            if (
                ...
                raise AppApiException(
                    500,
                    _("Model name【{model_name}】already exists").format(
    ...
    class Operate(serializers.Serializer):
        ...
        def delete(self, with_valid=True):
            ...
            if model.model_type == "LLM":
                ...
                dataset_count = Application.objects.filter(
                    tts_model_id=model_id
                ).count()
                if dataset_count > 0:
                    raise AppApiException(
                        500, f"该模型关联了{dataset_count} 个应用，无法删除该模型。"
                    )
