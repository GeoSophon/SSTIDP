...
class EmbedDocuments(serializers.Serializer):
    texts = serializers.ListField(
        ...
        child=serializers.CharField(
            required=True, error_messages=ErrMessage.char(_("vector text"))
        ),
...
class CompressDocument(serializers.Serializer):
    page_content = serializers.CharField(
        required=True, error_messages=ErrMessage.char(_("text"))
    )
    metadata = serializers.DictField(
        required=False, error_messages=ErrMessage.dict(_("metadata"))
    )
...
class CompressDocuments(serializers.Serializer):
    ...
    query = serializers.CharField(
        required=True, error_messages=ErrMessage.char(_("query"))
    )


class ModelApplySerializers(serializers.Serializer):
    model_id = serializers.UUIDField(
        required=True, error_messages=ErrMessage.uuid(_("model id"))
    )
