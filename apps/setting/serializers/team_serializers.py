...
def get_response_body_api():
    return openapi.Schema(
        ...
        properties={
            "id": openapi.Schema(
                type=openapi.TYPE_STRING, title=_("user id"), description=_("user id")
            ),
            "username": openapi.Schema(
                type=openapi.TYPE_STRING, title=_("Username"), description=_("Username")
            ),
            ...
            "team_id": openapi.Schema(
                type=openapi.TYPE_STRING, title=_("team id"), description=_("team id")
            ),
...
class TeamMemberPermissionOperate(ApiMixin, serializers.Serializer):
    USE = serializers.BooleanField(
        required=True, error_messages=ErrMessage.boolean(_("use"))
    )
...
class UpdateTeamMemberItemPermissionSerializer(ApiMixin, serializers.Serializer):
    target_id = serializers.CharField(
        required=True, error_messages=ErrMessage.char(_("target id"))
    )
    type = serializers.CharField(
        required=True, error_messages=ErrMessage.char(_("type"))
    )
        ...
        return openapi.Schema(
            ...
            properties={
                "target_id": openapi.Schema(
                    type=openapi.TYPE_STRING,
                    title=_("dataset id/application id"),
    ...
    def is_valid(self, *, user_id=None):
        ...
        if illegal_target_id_list is not None and len(illegal_target_id_list) > 0:
            raise AppApiException(
                500,
                _("Non-existent application|knowledge base id[")
                + str(illegal_target_id_list)
                + "]",
            )
...
class TeamMemberSerializer(ApiMixin, serializers.Serializer):
    team_id = serializers.UUIDField(
        required=True, error_messages=ErrMessage.uuid(_("team id"))
    )
    ...
    def batch_add_member(self, user_id_list: List[str], with_valid=True):
        ...
        QuerySet(TeamMember).bulk_create(
            [
                team_member
                for team_member in create_team_member_list
    ...
    def add_member(self, username_or_email: str, with_valid=True):
        ...
        if QuerySet(TeamMember).filter(
            ...
            raise AppApiException(
                500,
                _(
                    "The current members already exist in the team, do not add them again."
                ),
            )
    ...
    class Operate(ApiMixin, serializers.Serializer):
        ...
        member_id = serializers.CharField(
            required=True, error_messages=ErrMessage.char(_("member id"))
        )
        # 团队id
        team_id = serializers.CharField(
            required=True, error_messages=ErrMessage.char(_("team id"))
        )
        ...
        def is_valid(self, *, raise_exception=True):
            ...
            if (
                ...
                raise AppApiException(
                    500, _("The member does not exist, please add a member first")
                )
        ...
        def edit(self, member_permission: Dict):
            ...
            if member_id == "root":
                raise AppApiException(
                    500, _("Administrator rights do not allow modification")
                )
