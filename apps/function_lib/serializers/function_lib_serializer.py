...
class FunctionLibInputField(serializers.Serializer):
    name = serializers.CharField(
        required=True, error_messages=ErrMessage.char(_("variable name"))
    )
    is_required = serializers.BooleanField(
        required=True, error_messages=ErrMessage.boolean(_("required"))
    )
    ...
    source = serializers.CharField(
        required=True,
        error_messages=ErrMessage.char(_("source")),
        validators=[
            validators.RegexValidator(
...
class DebugInstance(serializers.Serializer):
    ...
    code = serializers.CharField(
        required=True, error_messages=ErrMessage.char(_("function content"))
    )
...
class EditFunctionLib(serializers.Serializer):
    ...
    is_active = serializers.BooleanField(
        required=False, error_messages=ErrMessage.char(_("Is active"))
    )


class CreateFunctionLib(serializers.Serializer):
    name = serializers.CharField(
        required=True, error_messages=ErrMessage.char(_("function name"))
    )
    ...
    permission_type = serializers.CharField(
        required=True,
        error_messages=ErrMessage.char(_("permission")),
        validators=[
            validators.RegexValidator(
...
class FunctionLibSerializer(serializers.Serializer):
    class Query(serializers.Serializer):
        ...
        select_user_id = serializers.CharField(
            required=False, allow_null=True, allow_blank=True
        )
        function_type = serializers.CharField(
            required=False, allow_null=True, allow_blank=True
        )
        ...
        def get_query_set(self):
            ...
            if self.data.get("function_type") is not None:
                query_set = query_set.filter(
                    function_type=self.data.get("function_type")
                )
        ...
        def page(self, current_page: int, page_size: int, with_valid=True):
            ...
            def post_records_handler(row):
                return {**FunctionLibModelSerializer(row).data, "init_params": None}
    ...
    class Operate(serializers.Serializer):
        id = serializers.UUIDField(
            required=True, error_messages=ErrMessage.uuid(_("function id"))
        )
        user_id = serializers.UUIDField(
            required=True, error_messages=ErrMessage.uuid(_("user id"))
        )
        ...
        def edit(self, instance, with_valid=True):
            ...
            if "init_params" in edit_dict:
                ...
                if function_lib.init_params:
                    old_init_params = json.loads(
                        rsa_long_decrypt(function_lib.init_params)
                    )
                    for key in edit_dict["init_params"]:
        ...
        def export(self, with_valid=True):
            try:
                ...
                response["Content-Disposition"] = (
                    f'attachment; filename="{function_lib.name}.fx"'
                )
                return response
    ...
    class Import(serializers.Serializer):
        ...
        def import_(self, with_valid=True):
            ...
            function_lib_model = FunctionLib(
                id=uuid.uuid1(),
    ...
    class IconOperate(serializers.Serializer):
        ...
        def edit(self, with_valid=True):
            ...
            if self.data.get("image") is None:
                ...
                file_id = uuid.uuid1()
                file = File(
                    id=file_id, file_name=self.data.get("image").name, meta=meta
                )
    ...
    class InternalFunction(serializers.Serializer):
        ...
        def add(self, with_valid=True):
            ...
            internal_function_lib = (
                QuerySet(FunctionLib).filter(id=self.data.get("id")).first()
            )
