...
from dataset.task.tools import (
    get_save_handler,
...
def sync_web_dataset(dataset_id: str, url: str, selector: str):
    try:
        max_kb.info(
            _("Start--->Start synchronization web knowledge base:{dataset_id}").format(
        ...
        max_kb.info(
            _("End--->End synchronization web knowledge base:{dataset_id}").format(
                dataset_id=dataset_id
            )
        )
        ...
        max_kb_error.error(
            _(
                "Synchronize web knowledge base:{dataset_id} error{error}{traceback}"
            ).format(
...
@celery_app.task(
    base=QueueOnce,
    once={"keys": ["dataset_id"]},
    name="celery:sync_replace_web_dataset",
)
def sync_replace_web_dataset(dataset_id: str, url: str, selector: str):
    try:
        ...
        ForkManage(url, selector.split(" ") if selector is not None else []).fork(
            2, set(), get_sync_handler(dataset_id)
        )
        max_kb.info(
            _("End--->End synchronization web knowledge base:{dataset_id}").format(
                dataset_id=dataset_id
