...
from common.handle.impl.table.csv_parse_table_handle import (
    CsvSplitHandle as CsvSplitTableHandle,
)
from common.handle.impl.table.xls_parse_table_handle import (
    XlsSplitHandle as XlsSplitTableHandle,
)
from common.handle.impl.table.xlsx_parse_table_handle import (
    XlsxSplitHandle as XlsxSplitTableHandle,
)
...
class BatchCancelInstanceSerializer(serializers.Serializer):
    ...
    type = serializers.IntegerField(
        required=True, error_messages=ErrMessage.integer(_("task type"))
    )
...
class DocumentEditInstanceSerializer(ApiMixin, serializers.Serializer):
    ...
    directly_return_similarity = serializers.FloatField(
        ...
        max_value=2,
        min_value=0,
        error_messages=ErrMessage.float(_("directly return similarity")),
    )

    is_active = serializers.BooleanField(
        required=False, error_messages=ErrMessage.boolean(_("document is active"))
    )
...
class DocumentWebInstanceSerializer(ApiMixin, serializers.Serializer):
    source_url_list = serializers.ListField(
        ...
        child=serializers.CharField(
            required=True, error_messages=ErrMessage.char(_("document url list"))
        ),
        error_messages=ErrMessage.char(_("document url list")),
    )
    ...
    def get_request_body_api():
        return openapi.Schema(
            ...
            properties={
                "source_url_list": openapi.Schema(
                    type=openapi.TYPE_ARRAY,
                    title=_("source url list"),
...
class DocumentInstanceQASerializer(ApiMixin, serializers.Serializer):
    file_list = serializers.ListSerializer(
        ...
        child=serializers.FileField(
            required=True, error_messages=ErrMessage.file(_("file"))
        ),
    )
...
class DocumentSerializers(ApiMixin, serializers.Serializer):
    ...
    class Migrate(ApiMixin, serializers.Serializer):
        dataset_id = serializers.UUIDField(
            required=True, error_messages=ErrMessage.char(_("dataset id"))
        )
        target_dataset_id = serializers.UUIDField(
            required=True, error_messages=ErrMessage.char(_("target dataset id"))
        )
        document_id_list = serializers.ListField(
            required=True,
            error_messages=ErrMessage.char(_("document list")),
    ...
    class Query(ApiMixin, serializers.Serializer):
        ...
        hit_handling_method = serializers.CharField(
            required=False, error_messages=ErrMessage.char(_("hit handling method"))
        )
        is_active = serializers.BooleanField(
            required=False, error_messages=ErrMessage.boolean(_("document is active"))
        )
        task_type = serializers.IntegerField(
            required=False, error_messages=ErrMessage.integer(_("task type"))
        )
        status = serializers.CharField(
            required=False, error_messages=ErrMessage.char(_("status"))
        )
        order_by = serializers.CharField(
            required=False, error_messages=ErrMessage.char(_("order by"))
        )
    ...
    class Sync(ApiMixin, serializers.Serializer):
        document_id = serializers.UUIDField(
            required=True, error_messages=ErrMessage.char(_("document id"))
        )
        ...
        def sync(self, with_valid=True, with_embedding=True):
            ...
            try:
                ...
                if result.status == 200:
                    ...
                    # 批量插入段落
                    QuerySet(Paragraph).bulk_create(paragraph_model_list) if len(
                        paragraph_model_list
                    ) > 0 else None
                    # 批量插入问题
                    QuerySet(Problem).bulk_create(problem_model_list) if len(
                        problem_model_list
                    ) > 0 else None
                    # 插入关联问题
    ...
    class Operate(ApiMixin, serializers.Serializer):
        ...
        def refresh(self, state_list=None, with_valid=True):
            ...
            if (
                ...
                raise AppApiException(
                    500,
                    _("No permission to use this model") + f"{embedding_model.name}",
                )
            document_id = self.data.get("document_id")
            ListenerManagement.update_status(
                QuerySet(Document).filter(id=document_id),
            ...
            return openapi.Schema(
                ...
                properties={
                    ...
                    "paragraph_count": openapi.Schema(
                        type=openapi.TYPE_INTEGER,
                        title="_('document count')",
                        description="_('document count')",
                        default=1,
                    ),
    ...
    class Split(ApiMixin, serializers.Serializer):
        file = serializers.ListField(
            required=True, error_messages=ErrMessage.list(_("file list"))
        )

        limit = serializers.IntegerField(
            required=False, error_messages=ErrMessage.integer(_("limit"))
        )

        patterns = serializers.ListField(
            required=False,
            child=serializers.CharField(
                required=True, error_messages=ErrMessage.char(_("patterns"))
            ),
            error_messages=ErrMessage.list(_("patterns")),
        )

        with_filter = serializers.BooleanField(
            required=False, error_messages=ErrMessage.boolean(_("Auto Clean"))
        )
        ...
        def is_valid(self, *, raise_exception=True):
            ...
            for f in files:
                if f.size > 1024 * 1024 * 100:
                    raise AppApiException(
                        500,
                        _("The maximum size of the uploaded file cannot exceed 100MB"),
                    )
        ...
        def get_request_params_api():
            return [
                ...
                openapi.Parameter(
                    ...
                    title=_("Segmented regular list"),
                    description=_("Segmented regular list"),
                ),
                openapi.Parameter(
                    name="with_filter",
                    in_=openapi.IN_FORM,
                    required=False,
                    type=openapi.TYPE_BOOLEAN,
                    title=_("Whether to clear special characters"),
    ...
    class Batch(ApiMixin, serializers.Serializer):
        ...
        def batch_save(self, instance_list: List[Dict], with_valid=True):
            ...
            # 插入文档
            QuerySet(Document).bulk_create(document_model_list) if len(
                document_model_list
            ) > 0 else None
            # 批量插入段落
        ...
        def generate_related(self, model_id, prompt, state_list=None, with_valid=True):
            ...
            try:
                generate_related_by_document_id.delay(
                    document_id, model_id, prompt, state_list
                )
            except AlreadyQueued:
                raise AppApiException(
