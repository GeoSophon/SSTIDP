...
class ParagraphSerializers(ApiMixin, serializers.Serializer):
            ...
            return openapi.Schema(
                ...
                properties={
                    ...
                    "content": openapi.Schema(
                        ...
                        description=_("question content"),
                        default=_("question content"),
                    ),
                    "hit_num": openapi.Schema(
                        type=openapi.TYPE_INTEGER,
                        title=_("hit num"),
                        description=_("hit num"),
                        default=1,
                    ),
    ...
    class Migrate(ApiMixin, serializers.Serializer):
        ...
        target_dataset_id = serializers.UUIDField(
            required=True, error_messages=ErrMessage.uuid(_("target dataset id"))
        )
        target_document_id = serializers.UUIDField(
            required=True, error_messages=ErrMessage.uuid(_("target document id"))
        )
        paragraph_id_list = serializers.ListField(
            required=True,
            error_messages=ErrMessage.char(_("paragraph id list")),
    ...
    class Operate(ApiMixin, serializers.Serializer):
        # 段落id
        paragraph_id = serializers.UUIDField(
            required=True, error_messages=ErrMessage.char(_("paragraph id"))
        )
        # 知识库id
        dataset_id = serializers.UUIDField(
        ...
        def edit(self, instance: Dict):
            ...
            if "problem_list" in instance:
                ...
                # 删除问题
                QuerySet(Problem).filter(
                    id__in=[row.id for row in delete_problem_list]
                ).delete() if len(delete_problem_list) > 0 else None
                # 插入新的问题
    ...
    class Query(ApiMixin, serializers.Serializer):
        ...
        title = serializers.CharField(
            required=False, error_messages=ErrMessage.char(_("section title"))
        )
            ...
            return openapi.Schema(
                ...
                properties={
                    ...
                    "trample_num": openapi.Schema(
                        type=openapi.TYPE_INTEGER,
                        title=_("Number of dislikes"),
                        description=_("Number of dislikes"),
                        default=1,
                    ),
                    "dataset_id": openapi.Schema(
                        type=openapi.TYPE_STRING,
        ...
        def batch_generate_related(self, instance: Dict, with_valid=True):
            ...
            try:
                generate_related_by_paragraph_id_list.delay(
                    document_id, paragraph_id_list, model_id, prompt
                )
