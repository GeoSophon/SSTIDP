...
class DataSetSerializers(serializers.ModelSerializer):
    ...
    class Application(ApiMixin, serializers.Serializer):
        ...
        def get_response_body_api():
            return openapi.Schema(
                ...
                properties={
                    "id": openapi.Schema(
                        type=openapi.TYPE_STRING, title="", description=_("id")
                    ),
                    "name": openapi.Schema(
                        type=openapi.TYPE_STRING,
                    ...
                    "desc": openapi.Schema(
                        type=openapi.TYPE_STRING,
                        title="_('application description')",
                    ...
                    "multiple_rounds_dialogue": openapi.Schema(
                        ...
                        description=_("Whether to start multiple rounds of dialogue"),
                    ),
                    "prologue": openapi.Schema(
                        type=openapi.TYPE_STRING,
                        title=_("opening remarks"),
    ...
    class Query(ApiMixin, serializers.Serializer):
        ...
        def get_query_set(self):
            ...
            if (
                ...
                query_set = query_set.filter(
                    **{"temp.user_id__exact": self.data.get("select_user_id")}
                )
    ...
    class Create(ApiMixin, serializers.Serializer):
        ...
        class CreateBaseSerializers(ApiMixin, serializers.Serializer):
            ...
            embedding_mode_id = serializers.UUIDField(
                required=True, error_messages=ErrMessage.uuid(_("embedding mode"))
            )
        ...
        class CreateQASerializers(serializers.Serializer):
            ...
            file_list = serializers.ListSerializer(
                ...
                child=serializers.FileField(
                    required=True, error_messages=ErrMessage.file(_("file list"))
                ),
            )
                ...
                return openapi.Schema(
                    ...
                    properties={
                        ...
                        "name": openapi.Schema(
                            ...
                            title=_("dataset name"),
                            description=_("dataset name"),
                            default=_("dataset name"),
                        ),
                        "desc": openapi.Schema(
                            type=openapi.TYPE_STRING,
                            title=_("dataset description"),
                            description=_("dataset description"),
                            default=_("dataset description"),
                        ),
                        "user_id": openapi.Schema(
                            type=openapi.TYPE_STRING,
                            title=_("user id"),
        ...
        class CreateWebSerializers(serializers.Serializer):
            ...
            source_url = serializers.CharField(
                required=True,
                error_messages=ErrMessage.char(_("web source url")),
            )
    ...
    class Edit(serializers.Serializer):
        ...
        application_id_list = serializers.ListSerializer(
            ...
            child=serializers.UUIDField(
                required=True, error_messages=ErrMessage.char(_("application id"))
            ),
    ...
    class HitTest(ApiMixin, serializers.Serializer):
        ...
        user_id = serializers.UUIDField(
            required=False, error_messages=ErrMessage.char(_("user id"))
        )
    ...
    class SyncWeb(ApiMixin, serializers.Serializer):
        ...
        sync_type = serializers.CharField(
            required=True,
            error_messages=ErrMessage.char(_(_("sync type"))),
            validators=[
                validators.RegexValidator(
                ...
                if response.status == 200:
                    try:
                        ...
                        first = (
                            QuerySet(Document)
                            .filter(
                                meta__source_url=child_link.url.strip(), dataset=dataset
                            )
                            .first()
                        )
    ...
    class Operate(ApiMixin, serializers.Serializer):
        ...
        def re_embedding(self, with_valid=True):
            ...
            ListenerManagement.get_aggregation_document_status_by_dataset_id(
                self.data.get("id")
            )()
                ...
                for dataset_id in application_id_list:
                    if not application_dataset_id_list.__contains__(dataset_id):
                        raise AppApiException(
                            500,
                            _(
                                "Unknown application id {dataset_id}, cannot be associated"
