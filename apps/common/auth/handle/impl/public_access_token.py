        ...
        if request.path != "/api/application/profile":
            ...
            if application_setting_model is not None and X_PACK_LICENSE_IS_VALID:
                application_setting = (
                    ...
                    .filter(application_id=str(application_access_token.application_id))
                    .first()
                )
                if application_setting.authentication:
                    ...
                    if authentication is None:
                        authentication = {}
                    if application_setting.authentication_value.get(
                        "type"
                    ) != authentication.get("type") or password_encrypt(
                        application_setting.authentication_value.get("value")
                    ) != authentication.get("value"):
                        raise ChatException(
