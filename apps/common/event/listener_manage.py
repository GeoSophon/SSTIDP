...
class ListenerManagement:
    ...
    def embedding_by_data_list(data_list: List, embedding_model: Embeddings):
        # 批量向量化
        VectorStore.get_embedding_vector().batch_save(
            data_list, embedding_model, lambda: True
        )
    ...
    def update_status(query_set: QuerySet, taskType: TaskType, state: State):
        exec_sql = get_file_content(
            os.path.join(
                PROJECT_DIR, "apps", "dataset", "sql", "update_paragraph_status.sql"
            )
        )
    ...
    def embedding_by_dataset(dataset_id, embedding_model: Embeddings):
        ...
        """
        max_kb.info(
            _("Start--->Embedding dataset: {dataset_id}").format(dataset_id=dataset_id)
        )
        try:
            ...
            max_kb_error.error(
                _("Vectorized dataset: {dataset_id} error {error} {traceback}").format(
                    dataset_id=dataset_id,
                    error=str(e),
                    traceback=traceback.format_exc(),
                )
            ...
            max_kb.info(
                _("End--->Embedding dataset: {dataset_id}").format(
                    dataset_id=dataset_id
                )
            )
