...
class LoginSerializer(ApiMixin, serializers.Serializer):
    username = serializers.CharField(
        required=True, error_messages=ErrMessage.char(_("Username"))
    )

    password = serializers.CharField(
        required=True, error_messages=ErrMessage.char(_("Password"))
    )
...
class RegisterSerializer(ApiMixin, serializers.Serializer):
    ...
    code = serializers.CharField(
        required=True, error_messages=ErrMessage.char(_("Verification code"))
    )
        ...
        return openapi.Schema(
            ...
            properties={
                ...
                "email": openapi.Schema(
                    type=openapi.TYPE_STRING, title=_("Email"), description=_("Email")
                ),
                "password": openapi.Schema(
                    type=openapi.TYPE_STRING,
                    title=_("Password"),
...
class SwitchLanguageSerializer(serializers.Serializer):
    ...
    language = serializers.CharField(
        required=True, error_messages=ErrMessage.char(_("language"))
    )
...
class UserProfile(ApiMixin):
    ...
    def get_user_profile(user: User):
        ...
        return {
            ...
            "is_edit_password": user.password == "d880e722c47a34d8e9fce789fc62389d"
            if user.role == "ADMIN"
            else False,
    ...
    def get_response_body_api():
        return openapi.Schema(
            ...
            properties={
                "id": openapi.Schema(
                    type=openapi.TYPE_STRING, title="ID", description="ID"
                ),
                "username": openapi.Schema(
                    type=openapi.TYPE_STRING,
                    title=_("Username"),
                ...
                "role": openapi.Schema(
                    type=openapi.TYPE_STRING, title=_("Role"), description=_("Role")
                ),
...
class UserSerializer(ApiMixin, serializers.ModelSerializer):
    ...
    def listByType(self, type, user_id):
        teamIds = TeamMember.objects.filter(user_id=user_id).values_list(
            "id", flat=True
        )
        targets = TeamMemberPermission.objects.filter(
            member_id__in=teamIds, auth_target_type=type, operate__contains=["USE"]
        ...
        if type == "DATASET":
            user_list = (
                DataSet.objects.filter(Q(id__in=targets) | Q(user_id=user_id))
                .prefetch_related(prefetch_users)
                .distinct("user_id")
            )
        elif type == "APPLICATION":
            user_list = (
                Application.objects.filter(Q(id__in=targets) | Q(user_id=user_id))
                .prefetch_related(prefetch_users)
                .distinct("user_id")
            )
        elif type == "FUNCTION":
        ...
        return openapi.Schema(
            ...
            properties={
                ...
                "phone": openapi.Schema(
                    type=openapi.TYPE_STRING, title=_("Phone"), description=_("Phone")
                ),
                "is_active": openapi.Schema(
                    type=openapi.TYPE_BOOLEAN,
                    title=_("Is active"),
                ...
                "source": openapi.Schema(
                    type=openapi.TYPE_STRING, title=_("Source"), description=_("Source")
                ),
                "nick_name": openapi.Schema(
                    type=openapi.TYPE_STRING, title=_("Name"), description=_("Name")
                ),
...
class UserManageSerializer(serializers.Serializer):
    ...
    class UserInstance(ApiMixin, serializers.Serializer):
        ...
        phone = serializers.CharField(
            required=False,
            error_messages=ErrMessage.char(_("Phone")),
            max_length=20,
            allow_null=True,
            allow_blank=True,
        )
    ...
    def save(self, instance, with_valid=True):
        ...
        user = User(
            ...
            nick_name=""
            if instance.get("nick_name") is None
            else instance.get("nick_name"),
