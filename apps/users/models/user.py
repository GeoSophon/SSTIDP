...
def to_dynamics_permission(group_type: str, operate: list[str], dynamic_tag: str):
    ...
    """
    return [
        Permission(group=Group[group_type], operate=Operate[o], dynamic_tag=dynamic_tag)
        for o in operate
    ]
...
def get_user_dynamics_permission(user_id: str):
    ...
    member_permission_list = select_list(
        get_file_content(
            os.path.join(
                PROJECT_DIR, "apps", "setting", "sql", "get_user_permission.sql"
            )
        ),
    ...
    for member_permission in member_permission_list:
        result += to_dynamics_permission(
            member_permission.get("type"),
            member_permission.get("operate"),
