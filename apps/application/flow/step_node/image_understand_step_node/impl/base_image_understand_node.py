...
class BaseImageUnderstandNode(IImageUnderstandNode):
    ...
    def execute(
        ...
        **kwargs,
        ...
        image_model = get_model_instance_by_model_user_id(
            model_id,
                ...
                return HumanMessage(
                    content=[
                        ...
                        *[
                            {
                                "type": "image_url",
                                "image_url": {"url": f"/api/file/{file_id}"},
                            }
        ...
        for data in chat_record.details.values():
            if self.node.id == data["node_id"] and "image_list" in data:
                ...
                if len(image_list) == 0 or data["dialogue_type"] == "WORKFLOW":
                    return HumanMessage(content=chat_record.problem_text)
                image_base64_list = [
                    file_id_to_base64(image.get("file_id")) for image in image_list
                ]
                return HumanMessage(
                    content=[
                        {"type": "text", "text": data["question"]},
            ...
            messages = [
                HumanMessage(
                    content=[
                        {
                            "type": "text",
                            "text": self.workflow_manage.generate_prompt(prompt),
                        },
                        *images,
                    ]
                )
