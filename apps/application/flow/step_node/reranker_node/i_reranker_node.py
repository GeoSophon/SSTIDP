...
class RerankerSettingSerializer(serializers.Serializer):
    ...
    top_n = serializers.IntegerField(
        required=True, error_messages=ErrMessage.integer(_("Reference segment number"))
    )
    # 相似度 0-1之间
    ...
    max_paragraph_char_number = serializers.IntegerField(
        ...
        error_messages=ErrMessage.float(
            _("Maximum number of words in a quoted segment")
        ),
    )
...
class IRerankerNode(INode):
    ...
    def execute(
        self, question, reranker_setting, reranker_list, reranker_model_id, **kwargs
    ) -> NodeResult:
        pass
