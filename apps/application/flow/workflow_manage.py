...
class Flow:
    ...
    def get_next_nodes(self, node: Node):
        ...
        if len(node_list) == 0 and not end_nodes.__contains__(node.type):
            raise AppApiException(500, _("The next node that does not exist"))
        return node_list
    ...
    def is_valid_model_params(self):
        ...
        for node in node_list:
            ...
            if model_params_setting is None:
                ...
                node.properties.get("node_data", {})["model_params_setting"] = (
                    model_params_setting
                )
...
class WorkflowManage:
    ...
    def run_block(self, language="zh"):
        ...
        self.work_flow_post_handler.handler(
            self.params["chat_id"], self.params["chat_record_id"], answer_text, self
        )
    ...
    def dependent_node_been_executed(self, node_id):
        ...
        return all(
            [
                any(
                    [
                        self.dependent_node(up_node_id, node)
                        for node in self.node_context
                    ]
                )
                for up_node_id in up_node_id_list
            ]
        )
    ...
    def get_next_node_list(self, current_node, current_node_result):
        ...
        if (
            ...
            for edge in self.flow.edges:
                if (
                    edge.sourceNodeId == current_node.id
                    and f"{edge.sourceNodeId}_{current_node_result.node_variable.get('branch_id')}_right"
                    == edge.sourceAnchorId
                ):
                    next_node = [
                        node for node in self.flow.nodes if node.id == edge.targetNodeId
                    ]
                    if len(next_node) == 0:
                        continue
