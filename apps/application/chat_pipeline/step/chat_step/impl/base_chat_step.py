...
def add_access_num(client_id=None, client_type=None, application_id=None):
    if (
        ...
        application_public_access_client = (
            ...
            .filter(client_id=client_id, application_id=application_id)
            .first()
        )
...
class BaseChatStep(IChatStep):
    ...
    def get_block_result(
        ...
        if (
            ...
            return AIMessage(
                no_references_setting.get("value").replace("{question}", problem_text)
            ), False
        if chat_model is None:
            return AIMessage(
                _(
    ...
    def execute_block(
        ...
        try:
            ...
            if "reasoning_content" in chat_result.response_metadata:
                reasoning_content = chat_result.response_metadata.get(
                    "reasoning_content", ""
                )
            else:
