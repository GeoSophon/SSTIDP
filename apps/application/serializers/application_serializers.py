...
from function_lib.serializers.function_lib_serializer import (
    FunctionLibSerializer,
    FunctionLibModelSerializer,
)
...
class ModelDatasetAssociation(serializers.Serializer):
    ...
    dataset_id_list = serializers.ListSerializer(
        ...
        child=serializers.UUIDField(
            required=True, error_messages=ErrMessage.uuid(_("Knowledge base id"))
        ),
        ...
        for dataset_id in dataset_id_list:
            if not exist_dataset_id_list.__contains__(dataset_id):
                raise AppApiException(
                    500, f"{_('The knowledge base id does not exist')}【{dataset_id}】"
                )
...
class DatasetSettingSerializer(serializers.Serializer):
    ...
    max_paragraph_char_number = serializers.IntegerField(
        ...
        min_value=500,
        max_value=100000,
        error_messages=ErrMessage.integer(_("Maximum number of quoted characters")),
    )
...
class ModelSettingSerializer(serializers.Serializer):
    ...
    reasoning_content_enable = serializers.BooleanField(
        required=False, error_messages=ErrMessage.char(_("Thinking process switch"))
    )
...
class ApplicationWorkflowSerializer(serializers.Serializer):
    ...
    work_flow = serializers.DictField(
        required=False, error_messages=ErrMessage.dict(_("Workflow Objects"))
    )
...
class ApplicationSerializer(serializers.Serializer):
    ...
    problem_optimization = serializers.BooleanField(
        required=True, error_messages=ErrMessage.boolean(_("Question completion"))
    )
    ...
    type = serializers.CharField(
        required=True,
        error_messages=ErrMessage.char(_("Application Type")),
        validators=[
            validators.RegexValidator(
    ...
    model_params_setting = serializers.DictField(
        required=False, error_messages=ErrMessage.dict(_("Model parameters"))
    )
    ...
    class Embed(serializers.Serializer):
        host = serializers.CharField(
            required=True, error_messages=ErrMessage.char(_("Host"))
        )
        protocol = serializers.CharField(
            required=True, error_messages=ErrMessage.char(_("protocol"))
        )
        token = serializers.CharField(
            required=True, error_messages=ErrMessage.char(_("token"))
        )
        ...
        def get_embed(self, with_valid=True, params=None):
            ...
            show_guide = "true"
            float_icon = (
                f"{self.data.get('protocol')}://{self.data.get('host')}/ui/MaxKB.gif"
            )
            xpack_cache = DBModelManage.get_model("xpack_cache")
            X_PACK_LICENSE_IS_VALID = (
                False
                if xpack_cache is None
                else xpack_cache.get("XPACK_LICENSE_IS_VALID", False)
            )
            # 获取接入的query参数
            query = self.get_query_api_input(
                application_access_token.application, params
            )
            float_location = {
    ...
    class AccessTokenSerializer(serializers.Serializer):
        application_id = serializers.UUIDField(
            required=True, error_messages=ErrMessage.boolean(_("Application ID"))
        )

        class AccessTokenEditSerializer(serializers.Serializer):
            access_token_reset = serializers.BooleanField(
                required=False, error_messages=ErrMessage.boolean(_("Reset Token"))
            )
            is_active = serializers.BooleanField(
                required=False, error_messages=ErrMessage.boolean(_("Is it enabled"))
            )
            ...
            white_list = (
                serializers.ListSerializer(
                    ...
                    child=serializers.CharField(
                        required=True, error_messages=ErrMessage.char(_("Whitelist"))
                    ),
        ...
        def edit(self, instance: Dict, with_valid=True):
            ...
            if application_setting_model is not None and X_PACK_LICENSE_IS_VALID:
                application_setting, _ = (
                    application_setting_model.objects.get_or_create(
                        application_id=self.data.get("application_id")
                    )
                )
                if (
                    application_setting is not None
                    and instance.get("authentication") is not None
    ...
    class Authentication(serializers.Serializer):
        access_token = serializers.CharField(
            required=True, error_messages=ErrMessage.char(_("access_token"))
        )
        ...
        def auth(self, request, with_valid=True):
            ...
            if (
                ...
                if authentication_value is not None:
                    # 认证用户token
                    self.auth_authentication_value(
                        authentication_value,
                        str(application_access_token.application_id),
                    )
                    authentication = {
                        "type": authentication_value.get("type"),
        ...
        def auth_authentication_value(self, authentication_value, application_id):
            ...
            if application_setting_model is not None and X_PACK_LICENSE_IS_VALID:
                ...
                if (
                    application_setting.authentication
                    and authentication_value is not None
                ):
        ...
        def auth_password(source_authentication_value, authentication_value):
            return source_authentication_value.get("value") == authentication_value.get(
                "value"
            )
    ...
    class Edit(serializers.Serializer):
        ...
        icon = serializers.CharField(
            required=False, allow_null=True, error_messages=ErrMessage.char(_("Icon"))
        )
    ...
    class HitTest(serializers.Serializer):
        ...
        user_id = serializers.UUIDField(
            required=False, error_messages=ErrMessage.uuid(_("User ID"))
        )
        query_text = serializers.CharField(
            required=True, error_messages=ErrMessage.char(_("Query text"))
        )
    ...
    class Query(serializers.Serializer):
        name = serializers.CharField(
            required=False, error_messages=ErrMessage.char(_("Application Name"))
        )

        desc = serializers.CharField(
            required=False, error_messages=ErrMessage.char(_("Application Description"))
        )

        user_id = serializers.UUIDField(
            required=True, error_messages=ErrMessage.uuid(_("User ID"))
        )
        select_user_id = serializers.UUIDField(
            required=False, error_messages=ErrMessage.uuid(_("Select User ID"))
        )
        ...
        def get_query_set(self):
            ...
            if (
                "select_user_id" in self.data
                and self.data.get("select_user_id") is not None
                and self.data.get("select_user_id") != "all"
            ):
                query_set = query_set.filter(
                    **{
                        "temp_application.user_id__exact": self.data.get(
                            "select_user_id"
                        )
                    }
                )
    ...
    class IconOperate(serializers.Serializer):
        ...
        image = UploadedImageField(
            required=True, error_messages=ErrMessage.image(_("picture"))
        )
    ...
    class Import(serializers.Serializer):
        file = UploadedFileField(
            required=True, error_messages=ErrMessage.image(_("file"))
        )
        user_id = serializers.UUIDField(
            required=True, error_messages=ErrMessage.uuid(_("User ID"))
        )
        ...
        def import_(self, with_valid=True):
            ...
            if len(function_lib_list) > 0:
                function_lib_id_list = [
                    function_lib.get("id") for function_lib in function_lib_list
                ]
                exits_function_lib_id_list = [
                    str(function_lib.id)
                    for function_lib in QuerySet(FunctionLib).filter(
                ...
                function_lib_list = [
                    function_lib
                    for function_lib in function_lib_list
                    if not exits_function_lib_id_list.__contains__(
                        function_lib.get("id")
                    )
                ]
            application_model = self.to_application(application, user_id)
            function_lib_model_list = [
                self.to_function_lib(f, user_id) for f in function_lib_list
            ]
            application_model.save()
            # 插入认证信息
            ApplicationAccessToken(
        ...
        def to_application(application, user_id):
            ...
            for node in work_flow.get("nodes", []):
                if node.get("type") == "search-dataset-node":
                    node.get("properties", {}).get("node_data", {})[
                        "dataset_id_list"
                    ] = []
            return Application(
                id=uuid.uuid1(),
    ...
    class Operate(serializers.Serializer):
        ...
        def export(self, with_valid=True):
            try:
                ...
                function_lib_id_list = [
                    ...
                    .get("node_data", {})
                    .get("function_lib_id")
                    for node in application.work_flow.get("nodes", [])
                    if node.get("type") == "function-lib-node"
                ]
                function_lib_list = []
                ...
                mk_instance = MKInstance(
                    application_dict,
                    [
                        FunctionLibModelSerializer(function_lib).data
                        for function_lib in function_lib_list
                    ],
                    "v1",
                )
        ...
        def edit(self, instance: Dict, with_valid=True):
            ...
            if instance.get("model_id") is None or len(instance.get("model_id")) == 0:
                ...
                if not model.is_permission(application.user_id):
                    message = lazy_format(
                        _("No permission to use this model:{model_name}"),
        ...
        def save_application_mapping(
            ...
            ) if len(dataset_id_list) > 0 else None
            ...
            for node in nodes:
                if node["id"] == "base-node":
                    ...
                    if "file_upload_setting" in node_data:
                        instance["file_upload_setting"] = node_data[
                            "file_upload_setting"
                        ]
                    break
        ...
        def speech_to_text(self, file, with_valid=True):
            ...
            if application.stt_model_enable:
                model = get_model_instance_by_model_user_id(
                    application.stt_model_id, application.user_id
                )
        ...
        def play_demo_text(self, form_data, with_valid=True):
            ...
            model = get_model_instance_by_model_user_id(
                tts_model_id, application.user_id, **form_data
            )
            return model.text_to_speech(text)
        ...
        def application_list(self, with_valid=True):
            ...
            application_user_id = (
                user_id if user_id == str(application.user_id) else None
            )
            ...
            serialized_data = ApplicationSerializerModel(
                all_applications, many=True
            ).data
            application = sorted(
                serialized_data, key=lambda x: (x["type"] != "WORK_FLOW", x["name"])
            )
        ...
        def get_application(self, app_id, with_valid=True):
            ...
            self.update_search_node(
                embed_application.work_flow,
                [str(dataset.get("id")) for dataset in dataset_list],
            )
            return {
                **ApplicationSerializer.Query.reset_application(
                    ApplicationSerializerModel(embed_application).data
                ),
                "dataset_id_list": dataset_id_list,
            }
    ...
    class ApplicationKeySerializer(serializers.Serializer):
        ...
        class Edit(serializers.Serializer):
            is_active = serializers.BooleanField(
                required=False, error_messages=ErrMessage.boolean(_("Availability"))
            )

            allow_cross_domain = serializers.BooleanField(
                required=False,
                error_messages=ErrMessage.boolean(_("Is cross-domain allowed")),
            )
            ...
            cross_domain_list = serializers.ListSerializer(
                ...
                child=serializers.CharField(
                    required=True,
                    error_messages=ErrMessage.char(_("Cross-domain address")),
                ),
        ...
        class Operate(serializers.Serializer):
            ...
            api_key_id = serializers.CharField(
                required=True, error_messages=ErrMessage.char(_("ApiKeyid"))
            )
