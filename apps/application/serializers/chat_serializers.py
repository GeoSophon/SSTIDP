...
class WorkFlowSerializers(serializers.Serializer):
    nodes = serializers.ListSerializer(
        child=serializers.DictField(), error_messages=ErrMessage.uuid(_("node"))
    )
    edges = serializers.ListSerializer(
        child=serializers.DictField(), error_messages=ErrMessage.uuid(_("Connection"))
    )
...
class ReAbstractInstanceSerializers(serializers.Serializer):
    abstract = serializers.CharField(
        required=True, error_messages=ErrMessage.char(_("abstract"))
    )
...
class ChatSerializers(serializers.Serializer):
    class Operate(serializers.Serializer):
        ...
        def re_abstract(self, instance, with_valid=True):
            if with_valid:
                self.is_valid(raise_exception=True)
                ReAbstractInstanceSerializers(data=instance).is_valid(
    ...
    class ClientChatHistory(serializers.Serializer):
        ...
        client_id = serializers.UUIDField(
            required=True, error_messages=ErrMessage.uuid(_("Client id"))
        )
    ...
    class Query(serializers.Serializer):
        abstract = serializers.CharField(
            required=False, error_messages=ErrMessage.char(_("summary"))
        )
        start_time = serializers.DateField(
            format="%Y-%m-%d", error_messages=ErrMessage.date(_("Start time"))
        )
        end_time = serializers.DateField(
            format="%Y-%m-%d", error_messages=ErrMessage.date(_("End time"))
        )
        ...
        comparer = serializers.CharField(
            required=False,
            error_messages=ErrMessage.char(_("Comparator")),
            validators=[
        ...
        def to_row(row: Dict):
            ...
            reference_paragraph = "\n----------\n".join(
                [
                    ChatSerializers.Query.paragraph_list_to_string(
    ...
    class OpenTempChat(serializers.Serializer):
        ...
        multiple_rounds_dialogue = serializers.BooleanField(
            required=True,
            error_messages=ErrMessage.boolean(_("Multi-round conversation")),
        )
...
class ChatRecordSerializer(serializers.Serializer):
    class Operate(serializers.Serializer):
        ...
        chat_record_id = serializers.UUIDField(
            required=True, error_messages=ErrMessage.uuid(_("Conversation record id"))
        )
        ...
        def is_valid(self, *, current_role=None, raise_exception=False):
            ...
            if application_access_token is None:
                raise AppApiException(
                    500,
                    gettext("Application authentication information does not exist"),
                )
            if (
                ...
                raise AppApiException(
                    500, gettext("Displaying knowledge sources is not enabled")
                )
    ...
    class Query(serializers.Serializer):
        ...
        def reset_chat_record(chat_record):
            ...
            if len(chat_record.improve_paragraph_id_list) > 0:
                ...
                if len(paragraph_model_list) < len(
    ...
    class ImproveSerializer(serializers.Serializer):
        ...
        content = serializers.CharField(
            required=True, error_messages=ErrMessage.char(_("Paragraph content"))
        )
    ...
    class Improve(serializers.Serializer):
        ...
        def improve(self, instance: Dict, with_valid=True):
            ...
            problem, _ = Problem.objects.get_or_create(
                content=problem_text, dataset_id=dataset_id
            )
    ...
    class PostImprove(serializers.Serializer):
        ...
        def is_valid(self, *, raise_exception=False):
            ...
            if not Document.objects.filter(
                id=self.data["document_id"], dataset_id=self.data["dataset_id"]
            ).exists():
                raise AppApiException(500, gettext("The document id is incorrect"))
        ...
        def post_improve(self, instance: Dict):
            ChatRecordSerializer.PostImprove(data=instance).is_valid(
                raise_exception=True
            )
            ...
            for chat_record in chat_record_list:
                ...
                problem, _ = Problem.objects.get_or_create(
                    content=chat_record.problem_text, dataset_id=dataset_id
                )
                problem_paragraph_mapping = ProblemParagraphMapping(
                    id=uuid.uuid1(),
                    dataset_id=dataset_id,
            ...
            # 批量保存聊天记录
            ChatRecord.objects.bulk_update(
                chat_record_list, ["improve_paragraph_id_list"]
            )
            update_document_char_length(document_id)
